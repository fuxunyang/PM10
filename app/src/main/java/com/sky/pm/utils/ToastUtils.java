...
//                break;
    ...
    private static void jumpLogin(Context context) {
        //进行存储token,userid
//                SPUtils.put(context, Constants.TELPHONE, phone);
//                SPUtils.put(context, Constants.PWD, pwd);
        SPUtils.put(context, Constants.USERID, "");//存储userid
        SPUtils.put(context, Constants.TOKEN, "");//存储token
        SPUtils.put(context, Constants.ISONLINE, false);//存储登录状态
//        JumpAct.jumpActivity(ActivityLifecycle.getInstance().getCurrentActivity(), LoginActivity.class);
    }
}